...
void Leaderboard::displayHiscores(){
    ...
    hiscoreFile.read(reinterpret_cast<char *>(&recorder), sizeof(recorder));
    for(int count=0;count<TOP5;count++){
        cout<<"***********************"<<endl;
        cout<<count+1<<". "<<recorder[count].name<<"\t"<<recorder[count].score<<endl;
    }
    cout<<"***********************";
}

bool Leaderboard::checkScores(){
    for(int count=0;count<TOP5;count++){
        if(hiScore>recorder[count].score){
            index = count;
            return true;
        }
        else
...
void Leaderboard::reevaluateScores(){
    for(int start = TOP5;start>index;start--){
        Record temp=recorder[start-1];
        recorder[start]=temp;
    }
}
...
void Leaderboard::registration(){
    ...
    cin.getline(recorder[index].name, NAME_SIZE);
    recorder[index].score=hiScore;
    hiscoresFile.open("hiscores.dat", ios::out | ios::binary);
